# -*- coding: utf-8 -*-
"""
Created on Thu Dec  5 14:34:40 2024
...
def sliding_window_validation(data, y_var, features, param_grid, train_years=7, test_years=1):
    ...
    for start_idx in range(0, len(years) - (train_years + test_years) + 1, step_years):
        ...
        for params in ParameterGrid(param_grid):
            try:
                model = LogisticRegression(**params, max_iter=5000, random_state=42, class_weight= 'balanced')
    ...
    for target_lag_option in ['without_lag', 'with_lag']:
        for econ_lag_option in ['no_econ', 'with_econ', 'with_econ_lag']:
            ...
            param_grid = {
                ...
                'solver': ['lbfgs', 'saga', 'liblinear'],  # Solver
                'tol': [1e-4,1e-3, 1e-2, 0.1]  # Tolerance for stopping criteria
            }
...
metrics_df.to_excel("C:/Users/vertt/Desktop/walk_forward_lr_metrics_with_features2.xlsx", index=False)
